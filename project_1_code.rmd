...
plot_route_change <- function(df, p_vals, n=2) {
  ...
  for (i in 1:n) {
    ...
    plot(route_df$days_since_Jan1_2010, route_df$log_duration,
...
group_controlled_pvals_null <- df_null %>%
  group_by(route) %>%
...
# Identify routes with p-val < 0.05
significant_routes_null <- group_controlled_pvals_null %>% filter(p_adjust < 0.05)
```
There is no significant route for the null dataset generated by random normal distribution for each route, with mean being the original mean of the route's log_duration and standard deviation being the original standard deviation of the route's log_duration.
...
# run the permutation task specific test on the nonnull dataset
group_controlled_pvals_nonnull <- df_nonnull %>%
  group_by(route) %>%
  summarize(p_val = permutation.task_specific_test(random, days_since_Jan1_2010, abscor_t, 
                                                   paste(member, weekend, hour_of_day, temp))) %>%
  # Adjust p_vals for multiple testing
...
# Identify routes with p-val < 0.05
significant_routes_nonnull <- group_controlled_pvals_nonnull %>% filter(p_adjust < 0.05)
  
```
All the 603 routes are significant for the nonnull dataset. The nonnull dataset is generated by making each route's log_duration to decrease in every later ride. 

```{r, cache = TRUE}
# half-null and half-nonnull dataset
df_halfnull <-df %>%
  group_by(route) %>%
...
# run the permutation task specific test on the nonnull dataset
group_controlled_pvals_halfhalf <- df_halfhalf %>%
  group_by(route) %>%
  summarize(p_val = permutation.task_specific_test(random, days_since_Jan1_2010, abscor_t, 
                                                   paste(member, weekend, hour_of_day, temp))) %>%
  # Adjust p_vals for multiple testing
...
significant_routes2 <- group_controlled_pvals2 %>% filter(p_adjust < 0.05)

# Plot P-value histogram and top routes
hist(group_controlled_pvals2$p_adjust, 
     xlab="p_value", main="P-value Histogram", breaks=20)
length(significant_routes2)
