...
df <- data.frame(duration = log(duration), station_start, station_end, 
  ...
  # Filter out routes with less than 500 records
  filter(length(duration) >= 500, !outliers(duration)) %>% 
  ungroup()
...
df %>%
  group_by(route) %>%
  summarize(num_rides = length(duration)) %>%
  ggplot(aes(x=num_rides)) +
  geom_histogram()
```
# Permutation test without any control
...
# Absolute value of correlation as the statistic
abscor_t <- function(duration, days_from_start) abs(cor(duration, days_from_start))

# Control for Membership status, day of the week (2), hour of the day(2), and temperature(4)
# parenthesis represents how many categories there are for each variable
group_controlled_pvals <- df %>%
  group_by(route) %>%
  summarize(p_val = permutation.task_specific_test(duration, days_since_Jan1_2010, abscor_t, 
                                                   paste(member, weekend, hour_of_day, temp))) %>%
  # Adjust p_vals for multiple testing
  mutate(p_adjust = p.adjust(p_val, method = 'fdr'))
...
group_controlled_mean_pvals <- df %>%
  group_by(route) %>%
  summarize(p_val = permutation.task_specific_test(duration, days_since_Jan1_2010, mean_t, 
                                                   paste(member, weekend, hour_of_day, temp))) %>%
  mutate(p_adjust = p.adjust(p_val, method = 'fdr'))
...
group_controlled_median_pvals <- df %>%
  group_by(route) %>%
  summarize(p_val = permutation.task_specific_test(duration, days_since_Jan1_2010, median_t, 
                                                   paste(member, weekend, hour_of_day, temp))) %>%
  mutate(p_adjust = p.adjust(p_val, method = 'fdr'))
...
>>>>>>> d44f7cd4e03ffe2e06d4e193aee5ae868f08575b
