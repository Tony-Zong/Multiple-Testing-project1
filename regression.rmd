...
prediction_2010_holdout_adjusted <- prediction_2010_holdout %>% 
  mutate(upr_adjusted = upr + -0.07 * (upr-fit)) %>% 
  mutate(lwr_adjusted = lwr - -0.07 * (fit-lwr))
```
...
Finally, we get a non-coverage rate of 0.06093166, which is greater than 0.05. 
...
```{r}
prediction_2011_adjusted %>% 
  group_by(route) %>% 
  mutate(total_ride_count = n()) %>% 
  ungroup() %>% 
  filter(true_duration>upr_adjusted | true_duration<lwr_adjusted) %>% 
  select(c(route, total_ride_count)) %>% 
  group_by(route) %>% 
  mutate(noncoverage_count = n()) %>% 
  ungroup() %>% 
  distinct() %>% 
