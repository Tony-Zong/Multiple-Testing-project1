...
We want to make sure for each route we train a model on, there is enough data. Since we use 2010 data to build the regression model, we filter for "busy" routes based on 2010's data. Here, we decide to only consider the routes with more than 100 rides during September - December in 2010.
...
Then, we make predictions for the 2010 hold-out set, and construct 95% prediction intervals adjusted by weights.

```{r}
prediction_2010_holdout <- data.frame(predict(lm_2010, newdata = df10_m_holdout_agg, 
                                   interval = "predict", level = 0.95, 
                                   weights = df10_m_holdout_agg$counts)) %>% 
  mutate(true_duration = df10_m_holdout_agg$log_duration)
```

The original 95% prediction intervals have a non-coverage rate of 0.04307692:
...
prediction_2010_holdout_adjusted <- prediction_2010_holdout %>% 
  mutate(upr_adjusted = upr + -0.049 * (upr-fit)) %>% 
  mutate(lwr_adjusted = lwr - -0.049 * (fit-lwr))
```
...
```{r}
noncoverage_rates_2010 <- prediction_2010_holdout_adjusted %>% 
  mutate(route = df10_m_holdout_agg$route) %>% 
  group_by(route) %>% 
...
That is, we adjust the prediction intervals by making it narrower by a factor of (1-0.07). Now, if the 2011's data doesn't have significant change from 2010's data, using this newly adjusted prediction interval, we should also expect to see a non-coverage rate of roughly 5%.
...
prediction_2011 <- data.frame(predict(lm_2010, newdata = df11_m_agg, 
                                   interval = "predict", level = 0.95, weights = df11_m_agg$counts)) %>% 
  mutate(true_duration = df11_m_agg$log_duration) %>% 
  mutate(route = df11_m_agg$route)
```
...
Finally, we get an average non-coverage rate of 0.05543958, which is greater than 0.05. 
...
```{r}
noncoverage_rates_2011 <- prediction_2011_adjusted %>% 
  group_by(route) %>% 
  mutate(total_ride_count = n()) %>% 
  mutate(noncoverage_count = sum(true_duration>upr_adjusted | true_duration<lwr_adjusted)) %>% 
  mutate(noncoverage_proportion = noncoverage_count / total_ride_count) %>% 
...
for (a_route in train_routes) {
  ...
  res = prop.test(c(num_noncoverage_2010,num_noncoverage_2011),
            c(total_2010,total_2011),
            alternative = "less")
  p_val = res$p.value
