...
busy_routes_m_2010 <- intersect(busy_routes_m, busy_routes_m_2010)
```


## Now, work on members' data.


```{r}
# split data by year and membership status
df11_m <- df %>% filter(year == 2011 & member == T & route %in% busy_routes_m) %>% 
  select(c(log_duration, year, month, day, hour, day_of_week, daily_temp, weekend, route))

df10_m <- df %>% filter(year == 2010 & member == T & route %in% busy_routes_m_2010)
```
...
```{r}
write.csv(final,file='single_m.csv')
```
